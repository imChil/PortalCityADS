...
//  PortalCityAds
//
//  Created by  Pavel Chilin on 22.10.2022.
//
...
extension UIView {
    ...
    func showToast(toastMessage:String,duration:CGFloat) {
        ...
        lblMessage.frame = CGRect(x:((self.bounds.size.width)/2) - ((expectedSizeTitle.width+16)/2) , y: (self.bounds.size.height/2) - ((expectedSizeTitle.height+16)/2), width: expectedSizeTitle.width+16, height: expectedSizeTitle.height+10)
        ...
        constraints.append(bgView.centerYAnchor.constraint(equalTo: self.topAnchor))
        constraints.append(bgView.centerXAnchor.constraint(equalTo: self.leadingAnchor))
        ...
        UIView.animateKeyframes(withDuration:TimeInterval(duration) , delay: 0, options: [] , animations: {
            ...
            sucess in
            UIView.animate(withDuration:TimeInterval(duration), delay: 4, options: [] , animations: {
                lblMessage.alpha = 0
                bgView.alpha = 0
            })
